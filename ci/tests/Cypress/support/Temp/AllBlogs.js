describe('Check all blogs', function() {
  it('loop over all pages', function(){
    cy.visit('https://computest.nl/nl/knowledge-platform/blog/')

    cy.get('ul').find('li.d--sm').invoke('text').then((text)=>{
      var numOfPages = text.substr(4,text.length)
      for(var i=1;i<numOfPages;i++){
        var href = 'https://wwww.computest.nl/nl/knowledge-platform/blog/?page='+ i
        blogsOnPage(href)
      }
    })
